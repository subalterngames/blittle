...
impl Rgb8Surface {
    ...
    pub const fn pixel_to_l8(pixel: &[u8; 3]) -> u8 {
        L32Surface::f32_to_u8(Self::pixel_to_l32(pixel))
    }

    pub const fn pixel_to_la8(pixel: &[u8; 3]) -> [u8; 2] {
        [Self::pixel_to_l8(pixel), 255]
    }

    pub const fn pixel_to_l32(pixel: &[u8; 3]) -> f32 {
        Self::grayscale(pixel[0], pixel[1], pixel[2])
    }

    pub const fn pixel_to_la32(pixel: &[u8; 3]) -> [f32; 2] {
        [Self::pixel_to_l32(pixel), 1.]
    }

    pub const fn pixel_to_rgba8(pixel: &[u8; 3]) -> [u8; 4] {
        [pixel[0], pixel[1], pixel[2], 255]
    }

    pub const fn pixel_to_zrgb8(pixel: &[u8; 3]) -> u32 {
    ...
    pub(crate) const fn pixel_and_alpha_to_rgba32(pixel: &[u8; 3], alpha: f32) -> Vec4 {
        Vec4::new(
            L8Surface::u8_to_f32(pixel[0]),
            L8Surface::u8_to_f32(pixel[1]),
            L8Surface::u8_to_f32(pixel[2]),
